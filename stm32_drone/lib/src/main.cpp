...
adv_timer TIMER3;
Adv_Timer_Struct TIMER3_Struct;
int16_t gyro_data[3];
...
int main(void) {
	...
	pinMode(GPIOA, 10, INPUT); //usart1 rx
	pinConfig(GPIOA, 10, INPUT_PUPD);
	writeHigh(GPIOA, 10);
	pinMode(GPIOA, 2, OUTPUT); //usart2 tx
	pinConfig(GPIOA, 2, AFO_PP);
	USART1_Struct.USARTx = USART1;
	USART2_Struct.USARTx = USART2;
	Serial1.Init(&USART1_Struct, 115200, USART_MODE_RX);
	Serial2.Init(&USART2_Struct, 230400, USART_MODE_TX);
	Serial1.receiveIT();

	TIMER3_Struct.TIMx = TIM3;
	TIMER3_Struct.PSC = F_CPU/1000000 - 1; //prescaler set so one tick is 1us
	TIMER3_Struct.ARR = 5000;
	TIMER3.Init(&TIMER3_Struct);
