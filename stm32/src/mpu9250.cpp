...
void mpu9250::fifo_enable_gyro() {
	...
	USART.print("FIFO_ENABLE REGISTER after: "); USART.println(I2C.read(addr, FIFO_EN), 16);
}
...
void mpu9250::read_gyro_data(int16_t* gyro_data) {
	I2C.read(addr, GYRO_XOUT_H, 6, raw_gyro_data);
...
void mpu9250::read_accel_data(int16_t accel_data[]) {
	I2C.read(addr, ACCEL_XOUT_H, 6, raw_accel_data);
...
int16_t mpu9250::read_temp_data() {
	I2C.read(addr, TEMP_OUT_H, 2, raw_temp_data);
