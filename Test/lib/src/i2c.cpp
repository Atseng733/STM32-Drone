...
void i2c::Init(I2C_Typedef* i2cx, uint32_t i2c_speed) {
	...
	if(I2Cx == I2C1) { //assuming pin remaps are all default
		I2C1_CLK_EN;
		pinMode(GPIOB, 6, OUTPUT); //scl
		pinConfig(GPIOB, 6, GPO_OD);
		pinMode(GPIOB, 7, OUTPUT); //sda
		pinConfig(GPIOB, 7, GPO_OD);
		writeHigh(GPIOB, 6);
		writeHigh(GPIOB, 7);
		
		while(!(digitalRead(GPIOB, 6) == 1));
		while(!(digitalRead(GPIOB, 7) == 1));

		writeLow(GPIOB, 7);
		while(!(digitalRead(GPIOB, 7) == 0));
		writeLow(GPIOB, 6);
		while(!(digitalRead(GPIOB, 6) == 0));
